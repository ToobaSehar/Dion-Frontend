...
function buildAdminInitials(fullName: string | undefined, email: string | undefined): string {
...
export default function AdminDashboard() {
  ...
  const [mainView, setMainView] = useState<AdminPortalFigmaMainView>('dashboard');
  ...
  const topBarInitials = buildAdminInitials(user.full_name, user.email ?? undefined);
  const profileInitials = topBarInitials;

  const dashboardSlot = loadingData ? (
    ...
    <div className="mx-auto max-w-7xl px-4 pb-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Manage all bookings, properties, and users across the platform</p>
      </div>
      ...
      <div className="card">
        ...
        <AdminBookingTable bookings={bookings} onConfirmBooking={handleConfirmBooking} />
      </div>
    </div>
  );

  return (
    <div className="font-avenir-regular flex h-svh min-h-0 w-full overflow-hidden bg-white text-[#0b1d37]">
      <ClientPortalSidebar
        portal="admin"
          ...
          {bookings.map((booking) => {
            ...
            return (
              <tr key={booking.id} className="hover:bg-gray-50">
                ...
                <td className="table-cell">
                  <div className="text-sm text-gray-500">{format(new Date(booking.created_at), 'MMM dd, yyyy')}</div>
                </td>
              </tr>
            );
          })}
