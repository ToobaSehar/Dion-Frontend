...
import {
  PARTNER_PORTAL_HUB_REQUESTS_IN_MY_AREA_HREF,
  PARTNER_PORTAL_PAYOUTS_HREF,
  type PartnerPortalFigmaMainView,
} from '@/components/client-portal-figma/partnerPortalFigmaMainView';
import { getPartnerAreaRequestById } from '@/components/client-portal-figma/PartnerRequestsInMyAreaView';
import { PartnerSubmitOfferView } from '@/components/client-portal-figma/PartnerSubmitOfferView';
...
export type PartnerSubmitOfferRouteClientProps = {
  className?: string;
  requestId: string;
};

/**
 * Partner portal shell for **Submit Offer** at `/partner/dashboard/submit-offer/[requestId]`.
 */
export function PartnerSubmitOfferRouteClient({ className, requestId }: PartnerSubmitOfferRouteClientProps) {
  ...
  const row = useMemo(() => getPartnerAreaRequestById(requestId), [requestId]);
  ...
  const goBackToRequests = useCallback(() => {
    router.push(PARTNER_PORTAL_HUB_REQUESTS_IN_MY_AREA_HREF);
  }, [router]);
    ...
    >
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col pt-6">
          {row ? (
            <PartnerSubmitOfferView row={row} onBack={goBackToRequests} />
          ) : (
            <div className="flex flex-col items-center justify-center gap-4 px-6 py-20">
              <p className="font-avenir-regular text-center text-sm text-[#717680]">This request could not be found.</p>
              <button
                type="button"
      ...
      {logoutOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="partner-submit-offer-logout-title"
        >
          <div className="w-full max-w-md rounded-2xl border border-[#e9eaeb] bg-white p-6 shadow-xl">
            <h2 id="partner-submit-offer-logout-title" className="font-avenir-regular text-lg font-semibold text-[#0B1D37]">
              Sign out?
            </h2>
