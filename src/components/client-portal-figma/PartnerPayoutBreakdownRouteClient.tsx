...
import { PartnerPayoutBreakdownView } from '@/components/client-portal-figma/PartnerPayoutBreakdownView';
import { getPartnerPayoutBreakdownByRowId } from '@/components/client-portal-figma/partnerPayoutBreakdownData';
import {
...
export type PartnerPayoutBreakdownRouteClientProps = {
  className?: string;
  payoutRowId: string;
};

/**
 * Partner shell for **Payout breakdown** at `/payouts/[payoutRowId]`.
 */
export function PartnerPayoutBreakdownRouteClient({ className, payoutRowId }: PartnerPayoutBreakdownRouteClientProps) {
  ...
  const detail = useMemo(() => getPartnerPayoutBreakdownByRowId(payoutRowId), [payoutRowId]);
  ...
  const goBackToPayouts = useCallback(() => {
    router.push(PARTNER_PORTAL_PAYOUTS_HREF);
  }, [router]);
    ...
    >
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col pt-6">
          {detail ? (
            <PartnerPayoutBreakdownView detail={detail} onBack={goBackToPayouts} />
          ) : (
            <div className="flex flex-col items-center justify-center gap-4 px-6 py-20">
              <p className="font-avenir-regular text-center text-sm text-[#717680]">
                This payout breakdown could not be found.
              </p>
              <button
                type="button"
                onClick={goBackToPayouts}
      ...
      {logoutOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="partner-payout-breakdown-logout-title"
        >
          <div className="w-full max-w-md rounded-2xl border border-[#e9eaeb] bg-white p-6 shadow-xl">
            <h2
              id="partner-payout-breakdown-logout-title"
              className="font-avenir-regular text-lg font-semibold text-[#0B1D37]"
            >
              Sign out?
            </h2>
