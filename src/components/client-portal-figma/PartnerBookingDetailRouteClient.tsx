...
import { PartnerBookingDetailView } from '@/components/client-portal-figma/PartnerBookingDetailView';
import { getPartnerBookingDetailByRowId } from '@/components/client-portal-figma/partnerBookingData';
import {
  PARTNER_PORTAL_HUB_MY_BOOKINGS_HREF,
...
export type PartnerBookingDetailRouteClientProps = {
  className?: string;
  bookingRowId: string;
};

/**
 * Partner shell for **Booking details** at `/partner/dashboard/bookings/[bookingRowId]`.
 */
export function PartnerBookingDetailRouteClient({ className, bookingRowId }: PartnerBookingDetailRouteClientProps) {
  ...
  const detail = useMemo(() => getPartnerBookingDetailByRowId(bookingRowId), [bookingRowId]);
  ...
  const goBackToBookings = useCallback(() => {
    router.push(PARTNER_PORTAL_HUB_MY_BOOKINGS_HREF);
  }, [router]);
    ...
    >
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col pt-6">
          {detail ? (
            <PartnerBookingDetailView detail={detail} onBack={goBackToBookings} />
          ) : (
            <div className="flex flex-col items-center justify-center gap-4 px-6 py-20">
              <p className="font-avenir-regular text-center text-sm text-[#717680]">This booking could not be found.</p>
              <button
                type="button"
      ...
      {logoutOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="partner-booking-detail-logout-title"
        >
          <div className="w-full max-w-md rounded-2xl border border-[#e9eaeb] bg-white p-6 shadow-xl">
            <h2 id="partner-booking-detail-logout-title" className="font-avenir-regular text-lg font-semibold text-[#0B1D37]">
              Sign out?
            </h2>
