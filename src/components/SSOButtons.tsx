'use client';

import { supabase } from '@/lib/supabase';
import { BookingHubSecondaryButton } from '@/components/booking-hub-button';

interface SSOButtonsProps {
  role: 'client' | 'partner';
  returnTo?: string;
  onBeforeRedirect?: () => void;
  /** @default 'sign-in' — keeps existing copy where callers omit this. */
  verb?: 'sign-in' | 'continue' | 'sign-up';
}

export function SSOButtons({ role, returnTo, onBeforeRedirect, verb = 'sign-in' }: SSOButtonsProps) {
  const buildRedirectTo = () => {
    const base = typeof window !== 'undefined' ? window.location.origin : '';
    const rt = returnTo ? `&return_to=${encodeURIComponent(returnTo)}` : '';
    return `${base}/auth/callback?role=${role}${rt}`;
  };

  const handleGoogle = async () => {
    onBeforeRedirect?.();
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: buildRedirectTo() },
    });
  };

  const handleMicrosoft = async () => {
    onBeforeRedirect?.();
    await supabase.auth.signInWithOAuth({
      provider: 'azure',
      options: { redirectTo: buildRedirectTo() },
    });
  };

  const googleLabel =
    verb === 'continue'
      ? 'Continue with Google'
      : verb === 'sign-up'
        ? 'Sign up with Google'
        : 'Sign in with Google';
  const microsoftLabel =
    verb === 'continue'
      ? 'Continue with Microsoft'
      : verb === 'sign-up'
        ? 'Sign up with Microsoft'
        : 'Sign in with Microsoft';

  const googleIcon = (
    <span className="inline-flex size-5 shrink-0 items-center justify-center [&_svg]:size-5">
      <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden>
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
      </svg>
    </span>
  );

  const microsoftIcon = (
    <span className="inline-flex size-5 shrink-0 items-center justify-center [&_svg]:size-5">
      <svg viewBox="0 0 21 21" xmlns="http://www.w3.org/2000/svg" aria-hidden>
        <rect x="1" y="1" width="9" height="9" fill="#F25022" />
        <rect x="11" y="1" width="9" height="9" fill="#7FBA00" />
        <rect x="1" y="11" width="9" height="9" fill="#00A4EF" />
        <rect x="11" y="11" width="9" height="9" fill="#FFB900" />
      </svg>
    </span>
  );

  return (
    <div className="space-y-3">
      <BookingHubSecondaryButton type="button" fullWidth responsive onClick={handleGoogle} iconLeading={googleIcon}>
        {googleLabel}
      </BookingHubSecondaryButton>

      <BookingHubSecondaryButton
        type="button"
        fullWidth
        responsive
        onClick={handleMicrosoft}
        iconLeading={microsoftIcon}
      >
        {microsoftLabel}
      </BookingHubSecondaryButton>
    </div>
  );
}

export function SSODivider({ centerLabel }: { centerLabel?: string }) {
  const label = centerLabel ?? 'or continue with email';
  return (
    <div className="relative my-5 sm:my-6">
      <div className="absolute inset-0 flex items-center">
        <div className="w-full border-t border-gray-300" />
      </div>
      <div className="relative flex justify-center text-xs sm:text-sm">
        <span
          className="bg-[#F6F6F4] px-4 text-gray-500"
          style={{ fontFamily: 'var(--font-avenir-regular)' }}
        >
          {label}
        </span>
      </div>
    </div>
  );
}
